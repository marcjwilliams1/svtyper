...
class SplitRead(object):
    ...
    def is_split_straddle(self,
        ...
        if (svtype == 'DEL' or svtype == 'INS'
            ...
            # reference_start-anchored junction).
            left_split, right_split = self.straddle_matching_orientation(
                    chrom_left, pos_left, chrom_right, pos_right, split_slop)
        elif svtype == 'BND':
            if o1_is_reverse == o2_is_reverse:
                # Both breakends resolve to the SAME inferred orientation -- an
                # inversion-shaped junction (a spec-compliant, common way to
                # represent an inversion as a BND pair). The all-sides check in
                # the else-branch would hand check_split_support the same
                # is_reverse for both ends and therefore only ever evaluate ONE
                # of reference_start/reference_end, silently dropping every
                # split/clip read whose informative boundary is the coordinate
                # never tested (issue #11). Use the same both-anchor geometry as
                # INV, which is exactly the "single inferred orientation"
                # situation.
                left_split, right_split = self.straddle_matching_orientation(
                        chrom_left, pos_left, chrom_right, pos_right, split_slop)
            else:
                # Mixed orientation (DEL/DUP-shaped BND): each end's orientation
                # is definite and distinct, so the all-sides check already
                # exercises both reference_start and reference_end across the two
                # breakends. Check all possible sides.
    ...
    def straddle_matching_orientation(self, chrom_left, pos_left,
                                      chrom_right, pos_right, split_slop):
        '''
        Split/clip geometry for a junction whose two breakends share the same
        inferred orientation (an inversion-shaped junction). Such a junction
        anchors supporting reads by BOTH reference_end and reference_start, so a
        single fixed is_reverse flag -- which is all check_split_support derives
        from a breakend's orientation -- would only ever test one coordinate and
        silently drop every read anchored on the other. Shared by SVTYPE=INV and
        by SVTYPE=BND pairs with o1_is_reverse == o2_is_reverse (issue #11).
        '''
        if self.is_soft_clip:
            ...
            # breakends. Testing both orientations here would give the arbitrary
            # free edge a spurious second chance to land on a breakend (a real
            # false positive for inversion-shaped clips; verified with a
            ...
            return (False, support) if left_clipped else (support, False)
        # Genuine SA-tag split: two real alignments meeting at a directly
        # observed junction (no arbitrary free edge), so test each piece against
        # each breakend in BOTH orientations to catch either inversion junction.
